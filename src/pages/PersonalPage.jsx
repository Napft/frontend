        ...
        <div className="gradient-bg-hero">
            ...
            <center>
                <div className="h-[400px] w-[40%] bg-[#eeeeee44] rounded-md flex justify-around">
                    <Identicon size={100} className="h-10 w-10 object-fill rounded-full " />
            ...
            <div className="min-h-[200px] bg-[#ffffff11] rounded-2xl m-[100px] p-[30px]">
                <center><h2 style={{ fontSize: "40px", color: "#ffffffaa" }}>My NFTs</h2></center>
                {
                    (nftDetailsList.length > 0)
                        ? <div className='mygrid'>{nftDetailsList.filter((val) => (connectedAccount == `${val.owner}`.toLowerCase())).map((ele, index, arr) => <MyNftTile className='myitem' key={index} NFT={ele} />)}</div>
